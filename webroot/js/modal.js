...
var Modal	=	new Class({
	...
	options: {
		...
		// when loading images, don't restrict the height
	...
	attach_links: function()
	{
		...
		document.addEvent('click:relay(a[rel~="modal"])', function(e) {
			...
			this.open(a.href, width, height, nodom, title);
		}.bind(this));
	},
	...
	open: function(href, width, height, nodomload, title)	// ... becomes function(href, options) // DERRR
	{
		...
		else if(href.match(/\.(jpg|jpeg|gif|png)$/i))
		{
			...
			var image_loaded	=	function() {
				...
				var height	=	this.options.auto_image_height ? 'auto' : Math.floor(cheight) + 'px';
				this.objects.container.setStyles({
				...
				// resizing
				this.open_complete('<img src="'+ href +'" title="'+ title.trim() +'" />', {width: img.width, height: this.options.auto_image_height ? '' : img.height});
			}.bind(this);
	...
	open_complete: function(res, args)
	{
		...
		if(args.height)
		{
			this.objects.content.setStyle('height', args.height);
		}
		
		if(args.width)
		{
			this.objects.content.setStyle('width', args.width);
		}
	...
	close: function(disable_close_events)
	{
		...
		this.objects.container.setStyles({ display: 'none' });
		this.objects.content.setStyles({ overflow: '' });
